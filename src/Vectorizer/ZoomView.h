...
protected:
  ZoomView();	// protected constructor used by dynamic creation
...
public:
  ...
  // Zooming functions
  typedef enum ZoomMode{MODE_ZOOMOFF, MODE_ZOOMIN, MODE_ZOOMOUT} ZoomMode_;
  void      SetZoomMode(ZoomMode_ zoomMode);
  ZoomMode_ GetZoomMode() {return m_zoomMode;};
  int       DoZoomIn    (CRect &rect);
  int       DoZoomIn    (CPoint *point = NULL, double delta = 1.5);
  int       DoZoomOut   (CPoint *point = NULL, double delta = 1.5);
  int       DoZoomFull  ();
  // bdelmee code change
...
#endif //_DEBUG
	virtual void OnDraw(CDC* pDC){}//ZoomView::OnDraw(pDC);}; // Pure Virtual
